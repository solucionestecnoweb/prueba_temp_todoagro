...
class PurchaseBook(models.TransientModel):
    ...
    def get_lines(self):
        xfind = self.env['account.move'].search([('type', 'in', ('in_invoice', 'in_refund', 'in_receipt')), ('date', '>=', self.date_from), ('date', '<=', self.date_to), ('state', '=', 'posted'), ('invoice_payment_state', '=', 'paid')])
        return xfind
